...
# Initialize OpenAI client for SambaNova (if key available)
if SAMBANOVA_API_KEY:
    try:
        ...
        st.error(f"Failed to initialize SambaNova client: {e}")
        client = None
else:
    client = None
...
with tab2:
    ...
    # Initialize chat
    if "messages" not in st.session_state:
        ai_status = "🤖 SambaNova Cloud AI" if client else "📚 Built-in Knowledge Only"
        st.session_state.messages = [
